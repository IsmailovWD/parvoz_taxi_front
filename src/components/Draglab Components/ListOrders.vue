<template>
  <div class="card" ref="el2" :style="style" style="max-height: calc(100vh - 75px);">
    ...
    <div v-if="bool" style="display: flex; justify-content: space-between; padding: 5px 10px; padding-bottom: 0;">
      <n-input clearable v-model:value="searchValue" placeholder="Qidiruv" style="width: 300px;" :onUpdate:value="SearchFunc">
        <template #prefix>
    ...
    <div v-if="bool" class="draglab_content">
      <div class="content">
        ...
        <div v-else v-wave style="width: 100%;" v-for="(item, index) in list" :key="index">
          <div class="list" :id="'id' + item.status_id">
            <span class="id">{{ index + 1 }}</span>
            <span class="number">{{ item.phone_number }}</span>
            <span class="number">{{ vaqt(item.created_date) }}</span>
            <span class="name">{{ item.tarif }}</span>
            <span class="name">
              <n-button style="width: 100%;" size="small" strong secondary type="primary" v-if="item.status_id == 1 || item.status_id == 6">
                {{ item.status }}
              </n-button>
